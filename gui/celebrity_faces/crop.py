...
def crop_face(img, shape):
    ...
    best_index = 0
    cur_square = 0
    for ind,face_ in enumerate(faces):
        face=rect(face_)
        if abs(face[1][0]-face[0][0])*abs(face[3][1]-face[0][1])>cur_square:
            best_index=ind
            cur_square=abs(face[1][0] - face[0][0]) * abs(face[3][1] - face[0][1])
    face = rect(faces[best_index])
    ymin = face[0][0]
    ymax = face[1][0]
    xmin = face[0][1]
    xmax = face[3][1]
    ...
    return transform.resize(img[int(max(ymin,0)):int(max(ymax,0)), int(max(xmin,0)):int(max(xmax,0))], shape, preserve_range=True)